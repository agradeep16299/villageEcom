...
function OrdersAdminPage() {
    ...
    function TableRow({ _id, count, owner, total,status, products, address ,phone}) {
        return (
            <tr>
                <td>{_id}</td>
                <td>{owner?.name}</td>
    ...
    return (
        <>
            <Table responsive striped bordered hover>
                ...
                <tbody >
                    <Pagination data={orders} RenderComponent={TableRow} pageLimit={1} dataLimit={100} tablePagination={true} />
                </tbody>
            </Table>
            ...
            <Modal show={show} onHide={handleClose}>
                ...
                {orderToShow.map((order) => (
                    <div className="order-details__container d-flex justify-content-around py-2">
                        <img src={order.pictures[0].url} style={{ maxWidth: 100, height: 200, objectFit: "cover" }} />
                        <p>
